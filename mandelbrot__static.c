...
int main(int argc, char* argv[]) {
    ...
    double communication_time = 0.0; // Variable to store communication time
    ...
    // Measure communication time using MPI_Barrier
    MPI_Barrier(MPI_COMM_WORLD); // Synchronize all processes

    if (rank == 0) {
        communication_time = MPI_Wtime() - end_time; // Communication time
    ...
    if (rank == 0) {
        FILE* file = fopen("mandelbrot_static_parallel.ppm", "wb");
        fprintf(file, "P6\n%d %d\n255\n", width, height);
